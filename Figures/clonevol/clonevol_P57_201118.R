...
###P57, 2 biopises, hist-transformed
#T1;LN_neck_tonsil
#T2:LN_mesenterial
...
pyclone_out<- read.table(file="ouput_pyclone/output_200X_i2/P57_200X_i2/tables/loci.tsv",sep="\t",header=T)
...
vafs = data.frame(cluster=P_case$cluster_id,
                  T1_vaf=(P_case$`GCF0150-0057-T01`/2)*100,
                  T2_vaf=(P_case$`GCF0150-0057-T02`/2)*100,
                  stringsAsFactors=F)
...
samples<-c("P57_1","P57_2")
samples1<-c("P57_1\nPrimary\nLN_neck_tonsil","P57_2\nRelapsed\nLN_mesenterial")
...
res = infer.clonal.models(variants=vafs, cluster.col.name="cluster", vaf.col.names=samples,
                          ...
                          subclonal.test="bootstrap", subclonal.test.model="non-parametric",
                          founding.cluster=1,ignore.clusters = c(5,7:9),
                          ,cluster.center="mean", num.boots=1000,
                          min.cluster.vaf=0.01, sum.p=0.01, alpha=0.01)


vafs_used<- subset(vafs, !cluster %in% c(5,7:9))
...
pdf("./clonevol/P57/P57_trees.pdf", useDingbats = FALSE)
...
pdf('./clonevol/P57/P57_fish_200x_pyclone_anno_loc.pdf', width=14, height=7)
...
pdf("./clonevol/P57/P57_box.pdf", width=3, height=3,useDingbats = FALSE, title='')
...
plot.pairwise(vafs_used, col.names = samples,
              out.prefix = './clonevol/P57/P57_variants.pairwise.plot')


pdf('./clonevol/P57/P57_flow.pdf')
...
####checking coverage f
Pcase<-do.call("rbind", lapply( list.files("input_pyclone_271018_newpara/200X/GCF0150-0057-N01_200X/",full=TRUE),
...
save.image("G:/FL_resequncing/FL_exome_final/fl_latest/11_pyclone/BED_bam/counts_new/pyclone_output_301018/P57.RData")
