...
pyclone_out<- read.table(file="ouput_pyclone/output_200X_i2/P6_2samples_i2/tables/loci.tsv",sep="\t",header=T)
...
vafs = data.frame(cluster=P_case$cluster_id,
                  T1_vaf=(P_case$`GCF0150-0006-T02`/2)*100,
                  T2_vaf=(P_case$`GCF0150-0006-T03`/2)*100,
                  stringsAsFactors=F)
...
res = infer.clonal.models(variants=vafs, cluster.col.name="cluster", vaf.col.names=samples,
                          ...
                          subclonal.test="bootstrap", subclonal.test.model="non-parametric",
                          founding.cluster=1,ignore.clusters = c(6:9),
                          ,cluster.center="mean", num.boots=1000,
                          min.cluster.vaf=0.01, sum.p=0.01, alpha=0.01)


vafs_used<- subset(vafs, !cluster %in% c(6:9))
...
pdf("./clonevol/P6/P6_trees.pdf", useDingbats = FALSE)
...
pdf('./clonevol/P6/P6_fish_200x_pyclone_anno_loc.pdf', width=14, height=7)
...
pdf("./clonevol/P6/P6_box.pdf", width=3, height=3,useDingbats = FALSE, title='')
...
plot.pairwise(vafs_used, col.names = samples,
              out.prefix = './clonevol/P6/P6_variants.pairwise.plot')


pdf('./clonevol/P6/P6_flow.pdf')
plot.cluster.flow(vafs_used, vaf.col.names = samples,
                  sample.names = c('Pretreatment', 'Relapsed'))
dev.off()


####checking coverage f
Pcase<-do.call("rbind", lapply( list.files("input_pyclone_271018_newpara/200X/GCF0150-0006-N01_200X/",full=TRUE),
...
save.image("G:/FL_resequncing/FL_exome_final/fl_latest/11_pyclone/BED_bam/counts_new/pyclone_output_301018/P6.RData")
