...
###P14, 2 biopsies,  2 biopsies, nFL, one of biopsies has ploidy issue. So use CNA from FASIT instead of ASCAT
...
pyclone_out<- read.table(file="ouput_pyclone/output_200X_i2/P14_200X_i2/tables/loci.tsv",sep="\t",header=T)
...
vafs = data.frame(cluster=P_case$cluster_id,
                  T1_vaf=(P_case$`GCF0150-0014-T01`/2)*100,
                  T2_vaf=(P_case$`GCF0150-0014-T02`/2)*100,
                  stringsAsFactors=F)
...
res = infer.clonal.models(variants=vafs, cluster.col.name="cluster", vaf.col.names=samples,
                          ...
                          subclonal.test="bootstrap", subclonal.test.model="non-parametric",
                          founding.cluster=1,ignore.clusters = c(3,7,8),
                          cluster.center="mean", num.boots=1000,
                          min.cluster.vaf=0.01, sum.p=0.01, alpha=0.01)
...
vafs_used<- subset(vafs, !cluster %in% c(3,7,8))

vafs_used$cluster[vafs_used$cluster=="6"]<-3
...
pdf("./clonevol/P14/P14_trees.pdf", useDingbats = FALSE)
...
pdf('./clonevol/P14/P14_fish_200x_pyclone_anno_loc.pdf', width=14, height=7)
...
pdf("./clonevol/P14/P14_box.pdf", width=3, height=3,useDingbats = FALSE, title='')
...
plot.pairwise(vafs_used, col.names = samples,
              out.prefix = './clonevol/P14/P14_variants.pairwise.plot')


pdf('./clonevol/P14/P14_flow.pdf')
...
P14_1<- read.table(file="input_pyclone_271018_newpara/200X/GCF0150-0014-N01_200X/GCF0150-0014-T01.txt",sep="\t",header=T)
P14_2<- read.table(file="input_pyclone_271018_newpara/200X/GCF0150-0014-N01_200X/GCF0150-0014-T02.txt",sep="\t",header=T)
Pcase<- rbind(P14_1, P14_2)
