...
pyclone_out<- read.table(file="ouput_pyclone/output_200X_i2/P9_200X_i2/tables/loci.tsv",sep="\t",header=T)
...
vafs = data.frame(cluster=P_case$cluster_id,
                  T1_vaf=(P_case$`GCF0150-0009-T01`/2)*100,
                  T2_vaf=(P_case$`GCF0150-0009-T02`/2)*100,
                  stringsAsFactors=F)
...
res = infer.clonal.models(variants=vafs, cluster.col.name="cluster", vaf.col.names=samples,
                          ...
                          subclonal.test="bootstrap", subclonal.test.model="non-parametric",
                          founding.cluster=1,ignore.clusters = c(6),
                          cluster.center="mean", num.boots=1000,
                          min.cluster.vaf=0.01, sum.p=0.01, alpha=0.01)
...
vafs_used<- subset(vafs, !cluster %in% c(6))
...
pdf("./clonevol/P9/P9_trees.pdf", useDingbats = FALSE)
...
pdf('./clonevol/P9/P_fish_200x_pyclone_anno_loc.pdf', width=14, height=7)
...
pdf("./clonevol/P9/P9_box.pdf", width=3, height=3,useDingbats = FALSE, title='')
...
plot.pairwise(vafs_used, col.names = samples,
              out.prefix = './clonevol/P9/P9_variants.pairwise.plot')


pdf('./clonevol/P9/P9_flow.pdf')
...
P9_1<- read.table(file="input_pyclone_271018_newpara/200X/GCF0150-0009-N01_200X/GCF0150-0009-T01.txt",sep="\t",header=T)
P9_2<- read.table(file="input_pyclone_271018_newpara/200X/GCF0150-0009-N01_200X/GCF0150-0009-T02.txt",sep="\t",header=T)
Pcase<- rbind(P9_1, P9_2)
