...
pyclone_out<- read.table(file="ouput_pyclone/output_200X_i2/P48_200X_i2/tables/loci.tsv",sep="\t",header=T)
...
vafs = data.frame(cluster=P_case$cluster_id,
                  T1_vaf=(P_case$`GCF0150-0048-T01`/2)*100,
                  T2_vaf=(P_case$`GCF0150-0048-T02`/2)*100,
                  stringsAsFactors=F)
...
vafs$cluster[vafs$cluster==9]<-1
vafs$cluster[vafs$cluster==max_id]<-9

samples<-c("P48_1","P48_2")
samples1<-c("P48_1\nPretreat","P48_2\nRelapsed")
samples2<-c("P48_1\nPretreat\nLN_right_neck","P48_2\nRelapsed\nLN_right_inguinal")
...
res = infer.clonal.models(variants=vafs, cluster.col.name="cluster", vaf.col.names=samples,
                          ...
                          subclonal.test="bootstrap", subclonal.test.model="non-parametric",
                          founding.cluster=1,ignore.clusters = c(4,7,10,11,12,14:16),
                          cluster.center="mean", num.boots=1000,
                          min.cluster.vaf=0.01, sum.p=0.01, alpha=0.01)


vafs_used<- subset(vafs, !cluster %in% c(4,7,10,11,12,14:16))
vafs_used$cluster[vafs_used$cluster==13]<-4
...
pdf("./clonevol/P48/P48_trees.pdf", useDingbats = FALSE)
...
pdf('./clonevol/P48/P48_fish_200x_pyclone_anno_loc.pdf', width=14, height=7)
...
pdf("./clonevol/P48/P48_box.pdf", width=3, height=3,useDingbats = FALSE, title='')
...
plot.pairwise(vafs_used, col.names = samples,
              out.prefix = './clonevol/P48/P48_variants.pairwise.plot')


pdf('./clonevol/P48/P48_flow.pdf')
...
####checking coverage f
Pcase<-do.call("rbind", lapply( list.files("input_pyclone_271018_newpara/200X/GCF0150-0048-N01_200X/",full=TRUE),
...
save.image("G:/FL_resequncing/FL_exome_final/fl_latest/11_pyclone/BED_bam/counts_new/pyclone_output_301018/P48.RData")
