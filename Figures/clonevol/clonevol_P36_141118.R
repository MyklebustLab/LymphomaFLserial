...
pyclone_out<- read.table(file="ouput_pyclone/output_200X_i2/P36_200X_i2/tables/loci.tsv",sep="\t",header=T)
...
vafs = data.frame(cluster=P_case$cluster_id,
                  T1_vaf=(P_case$`GCF0150-0036-T01`/2)*100,
                  T2_vaf=(P_case$`GCF0150-0036-T02`/2)*100,
                  stringsAsFactors=F)
...
samples<-c("P36_1","P36_2")
samples1<-c("P36_1\nPretreat_1","P36_2\nPretreat_2")
samples2<-c("P36_1\nPrimary\nLN_left_inguinal","P36_2\nPretreat\nLN_left_inguinal")
...
res = infer.clonal.models(variants=vafs, cluster.col.name="cluster", vaf.col.names=samples,
                          ...
                          founding.cluster=1,ignore.clusters = c(4,11,12),
                          cluster.center="mean", num.boots=1000,
                          min.cluster.vaf=0.01, sum.p=0.01, alpha=0.01)



vafs_used<- subset(vafs, !cluster %in% c(4,11,12))

vafs_used$cluster[vafs_used$cluster==10]<-4
...
pdf("./clonevol/P36/P36_trees.pdf", useDingbats = FALSE)
...
pdf('./clonevol/P36/P36_fish_200x_pyclone_anno_loc.pdf', width=14, height=7)
...
pdf("./clonevol/P36/P36_box.pdf", width=3, height=3,useDingbats = FALSE, title='')
...
plot.pairwise(vafs_used, col.names = samples,
              out.prefix = './clonevol/P36/P36_variants.pairwise.plot')


pdf('./clonevol/P36/P36_flow.pdf')
plot.cluster.flow(vafs, vaf.col.names = samples,
                  sample.names = c('Primary', 'Pretreatment'))
dev.off()


####checking coverage 
Pcase<-do.call("rbind", lapply( list.files("input_pyclone_271018_newpara/200X/GCF0150-0036-N01_200X/",full=TRUE),
...
save.image("G:/FL_resequncing/FL_exome_final/fl_latest/11_pyclone/BED_bam/counts_new/pyclone_output_301018/P36.RData")
