...
pyclone_out<- read.table(file="ouput_pyclone/output_200X_i2/P32_200X_i2/tables/loci.tsv",sep="\t",header=T)
...
vafs = data.frame(cluster=P_case$cluster_id,
                  T1_vaf=(P_case$`GCF0150-0032-T01`/2)*100,
                  T2_vaf=(P_case$`GCF0150-0032-T02`/2)*100,
                  T3_vaf=(P_case$`GCF0150-0032-T03`/2)*100,
                  stringsAsFactors=F)
...
samples<-c("P32_1","P32_2","P32_3")
samples1<-c("P32_1\nPrimary","P32_2\nRelapse", "P32_3\nTransformed")
samples2<-c("P32_1\nPrimary\nLN_left_inguinal","P32_2\nRelapse\nEN_unknown", "P32_3\nTransformed\nEN_left_lung")
...
res = infer.clonal.models(variants=vafs, cluster.col.name="cluster", vaf.col.names=samples,
                          ...
                          founding.cluster=1,ignore.clusters = c(7,11,13,14,16:46),
                          cluster.center="mean", num.boots=1000,
                          min.cluster.vaf=0.01, sum.p=0.01, alpha=0.01)



vafs_used<- subset(vafs, !cluster %in% c(7,11,13,14,16:46))

vafs_used$cluster[vafs_used$cluster==15]<-7
vafs_used$cluster[vafs_used$cluster==12]<-11
...
pdf("./clonevol/P32/P32_trees.pdf", useDingbats = FALSE)
...
pdf('./clonevol/P32/P32_fish_200x_pyclone_anno_loc.pdf', width=14, height=7)
for (i in 1:(length(fishes)-5)){
...
pdf("./clonevol/P32/P32_box.pdf", width=3, height=3,useDingbats = FALSE, title='')
...
plot.pairwise(vafs_used, col.names = samples,
              out.prefix = './clonevol/P32/P32_variants.pairwise.plot')


pdf('./clonevol/P32/P32_flow.pdf')
...
####checking coverage 
Pcase<-do.call("rbind", lapply( list.files("input_pyclone_271018_newpara/200X/GCF0150-0032-N01_200X/",full=TRUE),
...
save.image("G:/FL_resequncing/FL_exome_final/fl_latest/11_pyclone/BED_bam/counts_new/pyclone_output_301018/P32.RData")
