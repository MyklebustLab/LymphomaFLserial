...
pyclone_out<- read.table(file="ouput_pyclone/output_200X_i2/P27_200X_i2/tables/loci.tsv",sep="\t",header=T)
...
vafs = data.frame(cluster=P_case$cluster_id,
                  T1_vaf=(P_case$`GCF0150-0027-T01`/2)*100,
                  T2_vaf=(P_case$`GCF0150-0027-T02`/2)*100,
                  T3_vaf=(P_case$`GCF0150-0027-T03`/2)*100,
                  stringsAsFactors=F)
...
samples<-c("P27_1","P27_2", "P27_3")
samples1<-c("P27_1\nPrimary","P27_2\nRelapsed", "P27_3\nTransformed")
samples2<-c("P27_1\nPrimary\nLN_left_inguinal","P27_2\nRelapsed\nLN_right_inguinal", "P27_3\nTransformed\nEN_Retroperitoneal")
...
dir.create("./clonevol/P27")
##


##first: use all clusters, no consensus models

vafs$P27_2[vafs$cluster=="3"]<-vafs$P27_2[vafs$cluster=="3"]+ 1.2
...
res = infer.clonal.models(variants=vafs, cluster.col.name="cluster", vaf.col.names=samples,
                          ...
                          subclonal.test="bootstrap", subclonal.test.model="non-parametric",
                          founding.cluster=1,
                          cluster.center="mean", num.boots=1000,ignore.clusters = c(8:12),
                          min.cluster.vaf=0.01, sum.p=0.01, alpha=0.01)



vafs_used<- subset(vafs, !cluster %in% c(8:12))
...
pdf("./clonevol/P27/P27_trees.pdf", useDingbats = FALSE)
...
pdf('./clonevol/P27/P27_fish_200x_pyclone_anno_loc.pdf', width=14, height=7)
...
pdf("./clonevol/P27/P27_box.pdf", width=3, height=3,useDingbats = FALSE, title='')
...
plot.pairwise(vafs_used, col.names = samples,
              out.prefix = './clonevol/P27/P27_variants.pairwise.plot')


pdf('./clonevol/P27/P27_flow.pdf')
plot.cluster.flow(vafs, vaf.col.names = samples,
                  sample.names = c('Primary', 'Relapse', 'Transform'))
dev.off()


####checking coverage f
Pcase<-do.call("rbind", lapply( list.files("input_pyclone_271018_newpara/200X/GCF0150-0027-N01_200X/",full=TRUE),
...
save.image("G:/FL_resequncing/FL_exome_final/fl_latest/11_pyclone/BED_bam/counts_new/pyclone_output_301018/P27.RData")
