...
pyclone_out<- read.table(file="ouput_pyclone/output_200X_i2/P43_200X_i2/tables/loci.tsv",sep="\t",header=T)
...
vafs = data.frame(cluster=P_case$cluster_id,
                  T1_vaf=(P_case$`GCF0150-0043-T01`/2)*100,
                  T2_vaf=(P_case$`GCF0150-0043-T02`/2)*100,
                  T3_vaf=(P_case$`GCF0150-0043-T03`/2)*100,
                  stringsAsFactors=F)

vafs$cluster<- vafs$cluster+1

samples<-c("P43_1","P43_2", "P43_3")
samples1<-c("P43_1\nPrimary","P43_2\nRelapsed", "P43_3\nTransformed")
samples2<-c("P43_1\nPrimary\nLN_unknown","P43_2\nRelapsed\nLN_unknown", "P43_3\nTransformed\nLN_left_neck")
...
res = infer.clonal.models(variants=vafs, cluster.col.name="cluster", vaf.col.names=samples,
                          ...
                          subclonal.test="bootstrap", subclonal.test.model="non-parametric",
                          founding.cluster=1,ignore.clusters = c(8,10:13),
                          cluster.center="mean", num.boots=1000,
                          min.cluster.vaf=0.01, sum.p=0.01, alpha=0.01)

vafs_used<- subset(vafs, !cluster %in% c(8,10:13))
vafs_used$cluster[vafs_used$cluster==9]<-8
...
pdf("./clonevol/P43/P43_trees.pdf", useDingbats = FALSE)
...
pdf('./clonevol/P43/P43_fish_200x_pyclone_anno_loc.pdf', width=14, height=7)
...
pdf("./clonevol/P43/P43_box.pdf", width=3, height=3,useDingbats = FALSE, title='')
...
plot.pairwise(vafs_used, col.names = samples,
              out.prefix = './clonevol/P43/P43_variants.pairwise.plot')


pdf('./clonevol/P43/P43_flow.pdf')
...
####checking coverage 
Pcase<-do.call("rbind", lapply( list.files("input_pyclone_271018_newpara/200X/GCF0150-0043-N01_200X/",full=TRUE),
...
save.image("G:/FL_resequncing/FL_exome_final/fl_latest/11_pyclone/BED_bam/counts_new/pyclone_output_301018/P43.RData")
