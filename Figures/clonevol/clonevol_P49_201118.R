...
###P49, 2 biopsies, nFL, one of biopsies has ploidy issue. So use CNA from FASIT instead of ASCAT
#T1: LN_left_neck
#T2: EN_left_neck
...
pyclone_out<- read.table(file="ouput_pyclone/output_200X_i2/P49_200X_i2/tables/loci.tsv",sep="\t",header=T)
...
vafs = data.frame(cluster=P_case$cluster_id,
                  T1_vaf=(P_case$`GCF0150-0049-T01`/2)*100,
                  T2_vaf=(P_case$`GCF0150-0049-T02`/2)*100,
                  stringsAsFactors=F)
...
res = infer.clonal.models(variants=vafs, cluster.col.name="cluster", vaf.col.names=samples,
                          ...
                          subclonal.test="bootstrap", subclonal.test.model="non-parametric",
                          founding.cluster=1,ignore.clusters = c(4,5,7:10),
                          cluster.center="mean", num.boots=1000,
                          min.cluster.vaf=0.01, sum.p=0.01, alpha=0.01)


vafs_used<- subset(vafs, !cluster %in% c(4,5,7:10))
...
pdf("./clonevol/P49/P49_trees.pdf", useDingbats = FALSE)
...
pdf('./clonevol/P49/P49_fish_200x_pyclone_anno_loc.pdf', width=14, height=7)
...
pdf("./clonevol/P49/P49_box.pdf", width=3, height=3,useDingbats = FALSE, title='')
...
plot.pairwise(vafs_used, col.names = samples,
              out.prefix = './clonevol/P49/P49_variants.pairwise.plot')


pdf('./clonevol/P49/P49_flow.pdf')
...
####checking coverage f
Pcase<-do.call("rbind", lapply( list.files("input_pyclone_271018_newpara/200X/GCF0150-0049-N01_200X/",full=TRUE),
...
save.image("G:/FL_resequncing/FL_exome_final/fl_latest/11_pyclone/BED_bam/counts_new/pyclone_output_301018/P49.RData")
