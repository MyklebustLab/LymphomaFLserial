...
###P41, 2 biopsies, primary transform
#T1:LN_left_axilla
#T2:EN_left_iliac bone



setwd("G:/FL_resequncing/FL_exome_final/fl_latest/11_pyclone/BED_bam/counts_new/pyclone_output_301018")

pyclone_out<- read.table(file="ouput_pyclone/output_200X_i2/P41_200X_i2/tables/loci.tsv",sep="\t",header=T)
...
vafs = data.frame(cluster=P_case$cluster_id,
                  T1_vaf=(P_case$`GCF0150-0041-T01`/2)*100,
                  T2_vaf=(P_case$`GCF0150-0041-T02`/2)*100,
                  stringsAsFactors=F)
...
samples<-c("P41_1","P41_2")
samples1<-c("P41_1\nPrimary_Transf","P41_2\nTransformed")
samples2<-c("P41_1\nPrimary_Transf\nLN_left_axilla","P41_2\nTransformed\nEN_left_iliac bone")
...
res = infer.clonal.models(variants=vafs, cluster.col.name="cluster", vaf.col.names=samples,
                          ...
                          founding.cluster=1,ignore.clusters =c(5,7:11) ,
                          cluster.center="mean", num.boots=1000,
                          min.cluster.vaf=0.01, sum.p=0.01, alpha=0.01)



vafs_used<- subset(vafs, !cluster %in% c(5,7:11))
...
pdf("./clonevol/P41/P41_trees.pdf", useDingbats = FALSE)
...
pdf('./clonevol/P41/P41_fish_200x_pyclone_anno_loc.pdf', width=14, height=7)
...
pdf("./clonevol/P41/P41_box.pdf", width=3, height=3,useDingbats = FALSE, title='')
...
plot.pairwise(vafs_used, col.names = samples,
              out.prefix = './clonevol/P41/P41_variants.pairwise.plot')


pdf('./clonevol/P41/P41_flow.pdf')
...
####checking coverage 
Pcase<-do.call("rbind", lapply( list.files("input_pyclone_271018_newpara/200X/GCF0150-0041-N01_200X/",full=TRUE),
...
save.image("G:/FL_resequncing/FL_exome_final/fl_latest/11_pyclone/BED_bam/counts_new/pyclone_output_301018/P41.RData")
